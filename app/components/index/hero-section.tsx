...
export default function HeroSection() {
  return (
    <section className={`${gridPatternOverlay.grid_pattern_overlay} pt-20 laptop:pt-28 h-[calc(100vh-87px)] laptop:h-[calc(100vh-85px)] desktop:h-[calc(100vh-101px)] bg-[url("/hero-bg.png")] bg-no-repeat bg-bottom px-4 border border-grey-15 border-t-0`}>
        <h1 className='font-semibold text-[34px] laptop:text-5xl desktop:text-[68px] text-center'>A Digital Product Studio<br />that will Work</h1>
        <div className='mt-[30px]'>
          ...
          <div className='hidden laptop:visible bg-[#242424]/20 laptop:w-fit laptop:m-auto laptop:flex laptop:items-center laptop:gap-1.5 border border-grey-15 text-grey-60 rounded-lg desktop:rounded-xl laptop:px-[30px] laptop:py-[18px] desktop:px-10 desktop:py-6 laptop:text-lg desktop:text-[22px]'>
            <span>For</span>
            <HighlightedSpan>Startups</HighlightedSpan>,
            <HighlightedSpan>Enterprise leaders</HighlightedSpan>,
            <HighlightedSpan>Media & Publishers</HighlightedSpan>,
            <span>and</span>
            <HighlightedSpan>Social Good</HighlightedSpan>
          </div>
        </div>
...
function HighlightedSpan({ children }: { children: React.ReactNode }) {
  return (
    <span className='inline-block bg-grey-15 laptop:px-2.5 laptop:py-2 laptop:rounded-md desktop:px-2.5 desktop:py-3 desktop:rounded-lg text-white'>
      {children}
    </span>
  )
}
